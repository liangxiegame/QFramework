...
namespace QFramework.Core.Rx
{
    internal class StartObservable<T> : OperatorObservableBase<T>
    {
        ...
        public StartObservable(Func<T> function, TimeSpan? startAfter, Utils.Scheduler.IScheduler scheduler)
        ...
        public StartObservable(Action action, TimeSpan? startAfter, Utils.Scheduler.IScheduler scheduler)
            : base(scheduler == Utils.Scheduler.Scheduler.CurrentThread)
        {
            this.action = action;
            this.startAfter = startAfter;
