...
namespace QFramework.Unity.Editor {
    ...
    {
        const string URL_GITHUB_API_LATEST_RELEASE = "https://api.github.com/repos/sschmid/QFramework-CSharp/releases/latest";
        const string URL_GITHUB_RELEASES = "https://github.com/sschmid/QFramework-CSharp/releases";
        ...
        static string requestLatestRelease() {
            ...
            httpWebRequest.UserAgent = Environment.UserName + "sschmid/QFramework-CSharp/QFramework.Unity/CheckForUpdates";
        ...
        static void displayUpdateInfo(UpdateInfo info) {
            switch (info.updateState) {
                case UpdateState.UpdateAvailable:
                    if (EditorUtility.DisplayDialog("QFramework Update",
                            string.Format("A newer version of QFramework is available!\n\n" +
                ...
                case UpdateState.UpToDate:
                    EditorUtility.DisplayDialog("QFramework Update",
                        "QFramework is up to date (" + info.localVersionString + ")",
                        "Ok"
                    );
                    break;
                case UpdateState.AheadOfLatestRelease:
                    if (EditorUtility.DisplayDialog("QFramework Update",
                            string.Format("Your QFramework version seems to be newer than the latest release?!?\n\n" +
                            "Currently installed version: {0}\n" +
                ...
                case UpdateState.NoConnection:
                    if (EditorUtility.DisplayDialog("QFramework Update",
                            "Could not request latest QFramework version!\n\n" +
