...
namespace QFramework.Editor
{
	...
	public static class CustomHierarchy
	{
		...
		public static void OpenCustomHierarchy()
		{
			...
			if (HierarchySwitch) { EditorApplication.hierarchyWindowItemOnGUI += HierarchWindowOnGui; }
			else { EditorApplication.hierarchyWindowItemOnGUI -= HierarchWindowOnGui; }
		...
		// 综合以上，根据类型，绘制图标和文字
		private static void DrawRectIcon<T>(Rect selectionRect, GameObject go, Color textColor, ref int order,
		...
		static void HierarchWindowOnGui(int instanceId, Rect selectionRect)
		{
			...
			try
			{
				...
				//		new Rect(selectionRect.x - 2, selectionRect.y, selectionRect.width, selectionRect.height + 2),
				//		go.name, style);
				//}
			}
			catch (Exception) { }
	...
	public static class ExtensionMethods
	{
		...
		{
			if (!checkChildren) { return go.GetComponent<T>(); }
			else { return go.GetComponentsInChildren<T>().FirstOrDefault() != null; }
		}
	}
}