...
namespace QFramework.Editor
{
	internal static class AnimationClipLoader
	{
		...
		private static void 加载(string strFilePath, ModelImporter modelImporter)
		{
			...
			try
			{
				...
				using (Stream file = File.Open(strConfigFileFullPath, FileMode.Open))
				{
					using (StreamReader reader = new StreamReader(file))
					{
						...
						while (!reader.EndOfStream) { listConfigData.Add(reader.ReadLine()); }
					}
				}
			}
			catch
			...
			Dictionary<string, ModelImporterClipAnimation> dictAnimations =
				new Dictionary<string, ModelImporterClipAnimation>(32);
			...
			List<ModelImporterClipAnimation> listAnimations原 =
				new List<ModelImporterClipAnimation>(modelImporter.clipAnimations);
			...
			List<ModelImporterClipAnimation> listAnimations新 =
				new List<ModelImporterClipAnimation>(dictAnimations.Count);
			...
			foreach (KeyValuePair<string, ModelImporterClipAnimation> kvpClip in dictAnimations)
			{
				...
				if (!is重复) { listAnimations新.Add(kvpClip.Value); }
			}

			//刷新界面：更改Selection.activeObject为null再换回去
			Object modelObject = Selection.activeObject;
					...
					using (StreamWriter reader = new StreamWriter(file))
					{
						if (animations.Length > 0)
						{
							...
							for (int i = 0; i < animations.Length; i++)
							{
								...
								reader.WriteLine("{0:000}-{1:000}-{2}-{3}", animation.firstFrame, animation.lastFrame,
									animation.name, animation.wrapMode);
							}
						}
					}
				}
