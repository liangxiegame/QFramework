@tool
class_name GenerateSystemCode

const TEMPLATE: String = """class_name %sSystem extends AbstractSystem

const NAME:String = "%sSystem"
...
static func build_file_path(system_name:String,indie_folder:bool)->String:
	...
	if indie_folder:
		return "%s%s_system/%s_system.gd" % [DIR,system_file_name,system_file_name]
		
	return "%s%s_system.gd" % [DIR,system_file_name]
...
static func generate(system_name:String,editorPlugin:EditorPlugin,indie_folder:bool) -> void:
	...
	if indie_folder:
		folder_path = "%s%s_system" % [DIR,system_file_name]
