...
namespace QFramework.Migration.Unity.Editor
{
    public class MigrationWindow : EditorWindow
    {
        [MenuItem("Tools/QFramework/Migrate...", false, 1000)]
...
//            EntitasEditorLayout.ShowWindow<MigrationWindow>("QFramework Migration - " + CheckForUpdates.GetLocalVersion());
        ...
        void OnGUI()
        {
            _scrollViewPosition = EditorGUILayout.BeginScrollView(_scrollViewPosition);
            {
                ...
                for (int i = 0; i < _migrations.Length; i++)
                {
                    ...
                    _showMigration[i] =
                        EntitasEditorLayout.DrawSectionHeaderToggle(migration.version, _showMigration[i]);
                    if (_showMigration[i])
                    {
        ...
        static void migrate(IMigration migration, MigrationWindow window)
        {
            ...
            if (shouldMigrate)
            {
                ...
                var path = "Assets/";
                path = EditorUtility.OpenFolderPanel(migration.version + ": " + migration.workingDirectory, path,
                    string.Empty);
