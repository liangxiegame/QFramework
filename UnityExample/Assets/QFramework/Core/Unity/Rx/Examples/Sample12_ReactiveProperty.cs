...
namespace QFramework.Examples
{
    public class Sample12_ReactiveProperty : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            MyButton.onClick.AsObservable().Subscribe(_ => enemy.CurrentHp.Value -= 99).AddTo(this);

            // Toggle, Input etc as Observable(OnValueChangedAsObservable is helper for provide isOn value on subscribe)
            // SubscribeToInteractable is QFramework.UI Extension Method, same as .interactable = x)
            ...
            MyInput.OnValueChangeAsObservable()
                ...
                .SubscribeToText(MyText); // SubscribeToText is QFramework.UI Extension Method
            ...
            // initial text:)
            IntRxProp.SubscribeToText(MyText).AddTo(this);
        }
    }
