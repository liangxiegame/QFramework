    ...
    internal class CSharpTokenizer
    {
        ...
        private void ClassifyIdentifier(StringBuilder sb, string line, int start, int end)
        {
            ...
            if (end > s)
            {
                ...
                {
                    char first = line[s];
                    isType = first >= 'A' && first <= 'Z'; // PascalCase → 类型名
                }
            }
        ...
        {
            sb.Append("<color=").Append(color).Append(">");
            AppendEscaped(sb, line, start, end);
            sb.Append("</color>");
        }

        private static void AppendEscaped(StringBuilder sb, string line, int start, int end)
        {
            for (int k = start; k < end; k++)
            {
                AppendEscapedChar(sb, line[k]);
            }
        }
