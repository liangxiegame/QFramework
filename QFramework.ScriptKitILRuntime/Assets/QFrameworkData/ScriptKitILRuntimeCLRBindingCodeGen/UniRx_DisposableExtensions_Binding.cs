...
{
    unsafe class UniRx_DisposableExtensions_Binding
    {
        public static void Register(ILRuntime.Runtime.Enviorment.AppDomain app)
        {
            MethodBase method;
            Type[] args;
            Type type = typeof(UniRx.DisposableExtensions);
            ...
            if (genericMethods.TryGetValue("AddTo", out lst))
            {
                foreach(var m in lst)
                {
                    if(m.MatchGenericParameters(args, typeof(System.IDisposable), typeof(System.IDisposable), typeof(UnityEngine.GameObject)))
                    {
                        method = m.MakeGenericMethod(args);
                        app.RegisterCLRMethodRedirection(method, AddTo_0);
